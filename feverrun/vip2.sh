...
if [[ "${XTong}" == "openwrt" ]]; then
	...
	if [[ `docker --version | grep -c "version"` -eq '0' ]]; then
		...
		bash -c "$(curl -fsSL https://https://ghproxy.com/https://raw.githubusercontent.com/shidahuilang/QL-/main/docker.sh)"
...
if [[ `docker ps -a | grep -c "qinglong"` -ge '1' ]]; then
	...
	if [[ `docker exec -it qinglong bash -c "cat /ql/config/auth.json" | grep -c "\"token\""` -ge '1' ]]; then
		...
		echo
		sleep 5
		docker exec -it qinglong bash -c "$(curl -fsSL https://https://ghproxy.com/https://raw.githubusercontent.com/shidahuilang/QL-/main/feverrun.sh)"
